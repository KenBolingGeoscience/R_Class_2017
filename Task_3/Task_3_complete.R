...
#2) Create a vector that is the cyl column of mtcars in two different ways:
...
#3)	Create a data frame that contains all the columns of mtcars, but only with cars that weigh less than 3.0 OR more than 4.0 (weight is in the wt column)

mtcarswt <- mtcars[ ,6]
mtcarssubwt <- mtcarswt[mtcarswt < 3 | mtcarswt > 4]

#4)	AR 3.1.7.1: Fix the following common subsetting errors (note that mtcars is a dataset that is built into base R; you don't have to do anything special to load it:

fourcyl <- mtcars$cyl [mtcars$cyl == 4]
mtcars[1,4]
mtcars[mtcars$cyl <= 5, ]
mtcars[mtcars$cyl == 4 | mtcars$cyl == 6, ] #specified location within dataframe  
